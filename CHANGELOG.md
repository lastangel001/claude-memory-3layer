...
**Fixed**
- **CI was red on both OS since v6.17.0 — a bats teardown footgun, not product code.** `tests/codemap.bats` skips when `ctags`/`ripgrep` are absent (the case on GitHub runners), but its `teardown` ended with `[[ -n "${FIX:-}" ]] && rm -rf "$FIX"` — with `FIX` unset on a skipped test the `&&` chain's exit status is 1, and bats fails any test whose teardown returns non-zero. So every skipped codemap test reported `not ok`. Local runs passed because the maintainer's machine has ctags (tests ran, `FIX` set). Fixed with an explicit `return 0` in teardown (same guard added to the new `tests/mcp-recall.bats`). Lesson filed in gotchas.md.
